      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 bg-white dark:bg-surface-850 border border-surface-300 dark:border-surface-700 rounded-lg px-4 py-2.5 text-sm text-surface-800 dark:text-surface-200 placeholder-surface-400 dark:placeholder-surface-600 focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500/30"
          />
        ...
        {dict.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-surface-400 dark:text-surface-600">
            ...
            <p className="text-base">{t('dictionary.noTerms')}</p>
            <p className="text-sm mt-1 text-surface-500 dark:text-surface-700">{t('dictionary.noTermsHint')}</p>
          </div>
        ) : (
          <div className="flex flex-wrap gap-2">
            {dict.map((word) => (
              <div
                key={word}
                className="flex items-center gap-2 bg-white dark:bg-surface-850 border border-surface-200 dark:border-surface-800 rounded-lg px-3 py-1.5 group hover:border-red-500/30 transition-colors"
              >
                <span className="text-sm text-surface-700 dark:text-surface-300">{word}</span>
                <button
                  onClick={() => removeWord(word)}
                  className="text-surface-400 dark:text-surface-600 hover:text-red-400 transition-colors opacity-0 group-hover:opacity-100"
                >
        ...
        <div className="bg-brand-500/5 border border-brand-500/10 rounded-xl p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm text-brand-300 font-medium">{t('dictionary.howItWorks')}</p>
