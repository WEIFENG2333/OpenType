...
export function ResultPanel({ rawText, processedText, error }: ResultPanelProps) {
  ...
  return (
    <div className="bg-white dark:bg-surface-850 border border-surface-200 dark:border-surface-800 rounded-xl overflow-hidden animate-slide-up">
      {/* Tabs + copy */}
      <div className="flex items-center justify-between px-4 py-2 border-b border-surface-200 dark:border-surface-800/60">
        <div className="flex items-center gap-1">
          <TabButton active={!showRaw} onClick={() => setShowRaw(false)}>{t('recording.polished')}</TabButton>
          <TabButton active={showRaw} onClick={() => setShowRaw(true)}>{t('recording.raw')}</TabButton>
        </div>
        <Button variant="ghost" size="sm" onClick={handleCopy}>
          {copied ? (
            <><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="20 6 9 17 4 12"/></svg>{t('recording.copied')}</>
          ) : (
            <><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>{t('recording.copy')}</>
          )}
        </Button>
      </div>
      ...
      {/* Text content */}
      <div className="p-4">
        <p className="text-surface-800 dark:text-surface-200 text-[15px] leading-relaxed whitespace-pre-wrap">
  ...
  return (
    <button
      ...
      className={`px-3 py-1 text-xs font-medium rounded-md transition-colors
        ${active ? 'bg-brand-600/15 text-brand-400' : 'text-surface-500 hover:text-surface-600 dark:hover:text-surface-400 hover:bg-surface-100 dark:hover:bg-surface-800/60'}`}
    >
      {children}
    </button>
  );
