...
export function Sidebar({ current, onNavigate, weeklyWords = 0 }: SidebarProps) {
  ...
  return (
    <div className="w-[68px] bg-white dark:bg-surface-950 border-r border-surface-200 dark:border-surface-800/60 flex flex-col items-center py-3 gap-1">
      {navItems.map((item) => {
          ...
          <button
            ...
            className={`w-12 h-12 rounded-xl flex items-center justify-center transition-all duration-200 relative group
              ${current === item.id
                ...
                : 'text-surface-500 hover:text-surface-700 dark:hover:text-surface-300 hover:bg-surface-100 dark:hover:bg-surface-800/60'}`}
            title={label}
          >
            {item.icon}
            ...
            {/* Tooltip */}
            <div className="absolute left-full ml-2 px-2.5 py-1.5 bg-surface-100 dark:bg-surface-800 border border-surface-300 dark:border-surface-700 text-surface-800 dark:text-surface-200 text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-50 shadow-lg">
              {label}
            </div>
          </button>
        );
      ...
      <div className="mt-auto flex flex-col items-center gap-2 mb-1">
        <div className="w-10 h-px bg-surface-100 dark:bg-surface-800" />
        <div className="text-center">
          <p className="text-[10px] font-mono text-surface-400 dark:text-surface-600">{weeklyWords}</p>
          <p className="text-[9px] text-surface-500 dark:text-surface-700">{t('sidebar.words')}</p>
        </div>
      </div>
    </div>
  );
